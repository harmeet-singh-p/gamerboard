...
namespace GameProj
{
    public class Helpers
    {
        ...
        public static List<FrameworkElement> FindInallTree(Visual parent, IFinderMatchVisualHelper helper)
        {
            List<FrameworkElement> lst = new List<FrameworkElement>();

            FindDownInallTree(lst, parent, null, helper);
        ...
        public static void FindDownInallTree(List<FrameworkElement> lst, DependencyObject parent, DependencyObject ignore, IFinderMatchVisualHelper helper)
        {
            for (int i = 0; i < VisualTreeHelper.GetChildrenCount(parent); i++)
            {
                if (lst.Count > 0 && helper.dontstop) break;
                ...
                if (visual != ignore)
                {
                    FindDownInallTree(lst, visual, ignore, helper);
                }
            }
        }
        ...
        public static List<FrameworkElement> SingleFindallInTree(Visual parent, IFinderMatchVisualHelper helper)
        {
            helper.dontstop = false;

            List<FrameworkElement> lst = FindInallTree(parent, helper);    

            return lst;
        }

        public static List<FrameworkElement> FindVisualallElementUp(Visual parent, String ElementName)
        {
            return SingleFindallInTree(parent, new FinderMatchName(ElementName));
        }
